...
print "\n\n";
print "#############################################\n";
print "#  PERL script to run GATK CombineVariants  #\n";
print "#  (merges VCF files)                       #\n";
print "#############################################\n\n";
...
##################################################
# Make a string of the names for CombineVariants #
##################################################
print "\n\nThere are $no_of_files VCF files in this file of file names.\n\n";

for ($list_count=1;$list_count<=$no_of_files;$list_count++)
{
	$vcf_merge_string = $vcf_merge_string." -V:"."$vcf_file_array[$list_count]"." $vcf_file_array[$list_count]";
}

$command =  "/usr/bin/java16 -jar -Xmx4g -jar /opt/newgatk/GenomeAnalysisTK.jar -T CombineVariants -R /home/genetics/canfam2/canfam2.fasta $vcf_merge_string -o combine_variants_out.vcf";
print "$command\n";
system "$command";

print "\n\nFINISHED CombineVariants\n\n";
