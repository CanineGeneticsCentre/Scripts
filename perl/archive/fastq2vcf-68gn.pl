...
my $start_clean_time			= ""; # The start of "cleaning" the raw (f2b) BAM file to the "final" BAM file
...
print "    1: Use run_snpEff or run_VEP to annotate your variants.\n\n";
 ...
 while ($choice_ok eq "false")
{
		...
		if ($answer eq "y")
		{
			...
			if ($no_of_threads_bwa > 24){$no_of_threads_bwa = "24"}
			...
			if ($no_of_threads_gatk_ug_nct > 24){$no_of_threads_gatk_ug_nct = "24"}
			...
			if ($variant_caller eq "UnifiedGenotyper")
			{
				...
				if ($no_of_threads_gatk_ug_nt > 24){$no_of_threads_gatk_ug_nt = "24"}
			}
			...
			if ($no_of_threads_gatk_rtc_nt > 24){$no_of_threads_gatk_rtc_nt = "24"}
			...
			if ($no_of_threads_gatk_br_nct > 24){$no_of_threads_gatk_br_nct = "24"}
			...
			if ($no_of_threads_gatk_pr_nct > 24){$no_of_threads_gatk_pr_nct = "24"}
...
for ($loop_count=1;$loop_count <=$no_of_files;$loop_count++)
{
	...
	&run_unix_command("java $memory_string $temp_dir_string -jar /opt/picard/CollectInsertSizeMetrics.jar INPUT=$final_bam OUTPUT=out2.junk HISTOGRAM_FILE=$insert_size_pdf VALIDATION_STRINGENCY=LENIENT","Step 15 Plot insert size");
...
#java -Xmx45g  -Djava.io.tmpdir=/home/LANPARK/mboursnell/javatempdir -jar /opt/picard/CollectWgsMetrics.jar INPUT=ESS_ESS_26602_case_final.bam  OUTPUT=WGS_metrics.out  REFERENCE_SEQUENCE=/home/genetics/canfam3/canfam3.fasta STOP_AFTER=100000000   VALIDATION_STRINGENCY=LENIENT
	...
	&run_unix_command("java $memory_string $temp_dir_string -jar /opt/picard/CollectWgsMetrics.jar INPUT=$final_bam OUTPUT=$WGS_metrics_file REFERENCE_SEQUENCE=$ref STOP_AFTER=100000000 VALIDATION_STRINGENCY=LENIENT","Step 15 Plot insert size");
...
&log_time($make_gVCF_time,"Total time for making gVCF files");
