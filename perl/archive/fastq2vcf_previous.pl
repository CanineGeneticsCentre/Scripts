...
#                                                          #
############################################################
my $memory						= "60"; # memory setting for Java in gigabytes
my $no_of_threads_bwa			= "8";

my $no_of_threads_gatk_ug_nct	= "1";  # UnifiedGenotyper -nct (or HaplotypeCaller) (UG has to be 1)
my $no_of_threads_gatk_ug_nt	= "8";  # UnifiedGenotyper -nt (or HaplotypeCaller)

my $no_of_threads_gatk_pr_nct	= "8";  # PrintReads -nct                <== This one is used for PrintReads
my $no_of_threads_gatk_br_nct	= "8";  # BaseRecalibrator -nct          <== This one is used for BaseRecalibrator
my $no_of_threads_gatk_rtc_nt	= "16"; # RealignerTargetCreator         <== This one is used for RealignerTargetCreator
...
my $from 						= 'NGS_analysis@gen-x1404-ws01.aht.org.uk'; # Who e-mails come from
...
my $filter_mismatching_string	= " --filter_mismatching_base_and_quals"; # Fixes a probalem with mismatching qualities
...
# Set java...(depeneds on whether this is Workstation or samba64)
if ( index($version,"w") == -1 ){$javabin="/etc/alternatives/java"} else {$javabin="java"}
...
if ($warning eq "on")
{
	...
	print "fastq2vcf is currently being tested and refined on the Workstation.\n\n";
	print "It is also currently set up to use the full memory and CPU capacity\n";
	print "of the Workstation so is designed for only one run at a time.\n\n";
...
if ($use_defined_region eq "no")
{
	...
	if (($use_other_ref_sequence eq "no")  && (($ref_seq_name eq "canfam3") || ($ref_seq_name eq "equcab2")))
	{
		...
		{
			&print_message("Earler on you said you didn't want to focus on a region","message");
...
if ($read_file_method eq "multiple")
{
	...
	# List file names #
	###################
	if ($paired_ends_type eq "SE"){print "\nThere are $no_of_files FASTQ 'reads' files in this file of file names.\n\n";}
...
for ($list_count=1;$list_count<=$no_of_files;$list_count++)
{
	if ($paired_ends_type eq "SE")
	{
		&print_both("    FASTQ file $list_count	\t$reads1_file_array[$list_count]\n");
	}
	if ($paired_ends_type eq "PE")
	{
		&print_both("    Pair of FASTQ files $list_count	\t$reads1_file_array[$list_count]\t$reads2_file_array[$list_count]\n");
	}
}
...
for ($loop_count=1;$loop_count <=$no_of_files;$loop_count++)
{
	...
	if  ($paired_ends_type eq "SE")
	{
		...
		if ($bwa_alignment_method eq "mem")
		{
			...
			&run_unix_command("$bwa_path mem -M -t $no_of_threads_bwa $ref $fastq_file_1 > $aligned_sam","Converting FASTQ to SAM using bwa mem");
	...
	if  ($paired_ends_type eq "PE")
	{
		...
		if ($bwa_alignment_method eq "aln")
		{
			...
			&run_unix_command("$bwa_path aln -t $no_of_threads_bwa $ref $fastq_file_1 > $aln_sa1_sai","1a Run bwa to make SAI file 1");
			...
			&run_unix_command("$bwa_path aln -t $no_of_threads_bwa $ref $fastq_file_2 > $aln_sa2_sai","1b Run bwa to make SAI file 2");
		...
		if ($bwa_alignment_method eq "mem")
		{
			...
			&run_unix_command("$bwa_path mem -M -t $no_of_threads_bwa $ref $fastq_file_1 $fastq_file_2 > $aligned_sam","1 Converting FASTQ to SAM using bwa mem");
	...
	# What about using SortSam to sort the file before running step 3? --GN
	...
	&run_unix_command("$javabin $memory_string $temp_dir_string -jar /opt/picard/ValidateSamFile.jar I=$aligned_sorted_rg_bam O=$validate_f2b_out MODE=SUMMARY MAX_OPEN_TEMP_FILES=$max_open_temp_files VALIDATION_STRINGENCY=SILENT","4 Picard ValidateSamFile");
	...
	&run_unix_command ("$javabin $memory_string $temp_dir_string -jar /opt/picard/AddOrReplaceReadGroups.jar I=$bam_file O=$rg_bam SO=coordinate VALIDATION_STRINGENCY=LENIENT CREATE_INDEX=true RGID=$sample_name  RGLB=$lib  RGPL=illumina  RGPU=$sample_name  RGSM=$sample_name","6 Add ReadGroups information");
...
&log_time($make_raw_bam_time_total,"Total time to make raw BAM files");
	...
	if ($remove_duplicates eq "yes")
	{
		...
		&run_unix_command ("$javabin $memory_string $temp_dir_string -jar /opt/picard/MarkDuplicates.jar I=$rg_bam O=$runtitle_dedup_bam  VALIDATION_STRINGENCY=$picard_validation_stringency CREATE_INDEX=true M=$runtitle_sample_metrics","7 Mark Duplicates");
	...
	&run_unix_command ("$javabin $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -T RealignerTargetCreator $GATK_region_string -nt $no_of_threads_gatk_rtc_nt -R $ref -I $runtitle_dedup_bam -o $runtitle_dedup_bam_intervals -mismatch 0.0 -S $GATK_validation_stringency $GATK_fix_quals_string $filter_mismatching_string","8 RealignerTargetCreator");
	...
	&run_unix_command ("$javabin $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -T IndelRealigner $GATK_region_string -R $ref -I $runtitle_dedup_bam -targetIntervals $runtitle_dedup_bam_intervals -o $runtitle_clean_dedup_bam -S $GATK_validation_stringency $GATK_fix_quals_string $filter_mismatching_string","9 IndelRealigner");
	...
	&run_unix_command ("$javabin $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -T BaseRecalibrator $GATK_region_string -nct $no_of_threads_gatk_br_nct -R $ref -I $runtitle_clean_dedup_bam $GATK_known_sites_string -o $runtitle_sample_recal_grp -S $GATK_validation_stringency","10 BaseRecalibrator");
	...
	&run_unix_command ("$javabin $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -T PrintReads $GATK_region_string -I $runtitle_clean_dedup_bam  -nct $no_of_threads_gatk_pr_nct -R $ref  -BQSR $runtitle_sample_recal_grp  -o $runtitle_clean_dedup_recal_bam -S $GATK_validation_stringency","11 PrintReads");
	...
	if ($validate_bam_files eq "yes")
	{
		...
		&run_unix_command ("$javabin $memory_string $temp_dir_string -jar /opt/picard/ValidateSamFile.jar INPUT=$runtitle_clean_dedup_recal_bam OUTPUT=$validate_final_out VALIDATION_STRINGENCY=$picard_validation_stringency MODE=SUMMARY MAX_OUTPUT=100 MAX_OPEN_TEMP_FILES=$max_open_temp_files","12 Picard ValidateSamFile");
	...
	if ($use_defined_region eq "yes")
	{
		...
		&run_unix_command("$javabin $memory_string $temp_dir_string -jar /opt/picard/BuildBamIndex.jar I=$region_only_bam O=$region_only_bai VALIDATION_STRINGENCY=LENIENT","15 Make BAM index");
	...
	&run_unix_command("$javabin $memory_string $temp_dir_string -jar /opt/picard/CollectGcBiasMetrics.jar REFERENCE_SEQUENCE=$ref INPUT=$final_bam O=out1.junk CHART_OUTPUT=$gc_bias_pdf VALIDATION_STRINGENCY=LENIENT","16 calculate GC bias");
	...
	&run_unix_command("$javabin $memory_string $temp_dir_string -jar /opt/picard/CollectInsertSizeMetrics.jar INPUT=$final_bam O=out2.junk HISTOGRAM_FILE=$insert_size_pdf VALIDATION_STRINGENCY=LENIENT","17 Plot insert size");
...
if ($variant_caller eq "UnifiedGenotyper")
{
	...
	{
		&run_unix_command("$javabin $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -R $ref -T UnifiedGenotyper $GATK_region_string -glm BOTH $running_input_string -stand_emit_conf $stand_emit_conf -stand_call_conf $stand_call_conf -o $final_VCF_file -S $GATK_validation_stringency -nt $no_of_threads_gatk_ug_nt -nct $no_of_threads_gatk_ug_nct","Run GATK UnifiedGenotyper to call variants");	
	}
	if ($use_default_stand_values eq "true")
	{
		&run_unix_command("$javabin $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -R $ref -T UnifiedGenotyper $GATK_region_string -glm BOTH $running_input_string -o $final_VCF_file -S $GATK_validation_stringency -nt $no_of_threads_gatk_ug_nt -nct $no_of_threads_gatk_ug_nct","Run GATK UnifiedGenotyper to call SNP variants");	
	}
...
if ($variant_caller eq "HaplotypeCaller")
{
	if ($use_default_stand_values eq "false")
	{
		&run_unix_command("$javabin $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -pairHMM VECTOR_LOGLESS_CACHING -R $ref -T HaplotypeCaller $GATK_region_string  $running_input_string -stand_emit_conf $stand_emit_conf -stand_call_conf $stand_call_conf -o $final_VCF_file -S $GATK_validation_stringency -nt $no_of_threads_gatk_ug_nt -nct $no_of_threads_gatk_ug_nct","Run GATK HaplotypeCaller to call variants");	
	}
	if ($use_default_stand_values eq "true")
	{
		&run_unix_command("$javabin $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -pairHMM VECTOR_LOGLESS_CACHING -R $ref -T HaplotypeCaller $GATK_region_string  $running_input_string -o $final_VCF_file -S $GATK_validation_stringency -nt $no_of_threads_gatk_ug_nt -nct $no_of_threads_gatk_ug_nct","Run GATK HaplotypeCaller to call variants");	
	}
	&record_output_file_size ("$final_VCF_file");	
} # HC
...
if ($annotate_variants eq "yes")
{
	...
	{
		if ($use_defined_region eq "yes"){&run_unix_command("perl /home/genetics/scripts/run_variant_effect_predictor.pl --file $final_VCF_file --ref $species --chr $chromosome","");}
		if ($use_defined_region eq "no"){&run_unix_command("perl /home/genetics/scripts/run_variant_effect_predictor.pl --file $final_VCF_file --ref $species --chr none","");}
	}
	if ($effect_predictor eq "snpEff")
	{
		if ($use_defined_region eq "yes"){&run_unix_command("perl /home/genetics/scripts/run_snpEff.pl --file $final_VCF_file --ref $species --chr $chromosome","");}
		if ($use_defined_region eq "no"){&run_unix_command("perl /home/genetics/scripts/run_snpEff.pl --file $final_VCF_file --ref $species","");}
	}
	if ($effect_predictor eq "none")
	{
...
sub send_email_err_alert($;$) {
	...
	print MAIL "An error was detected during script processing.\n\n";
