...
##########################################################################################
#									                                                     #      
#	fastq2vcf_haploid : processes FASTQ files through a pipeline to end up as VCF files  #
#									                                                     #
##########################################################################################
...
{
	$memory						= "35"; # memory setting for Java in gigabytes
	$no_of_threads_bwa			= "4";
	$no_of_threads_gatk_ug_nct	= "1";  # UnifiedGenotyper -nct (or HaplotypeCaller) (UG has to be 1)
	$no_of_threads_gatk_ug_nt	= "8";  # UnifiedGenotyper -nt (but NOT HaplotypeCaller)
	$no_of_threads_gatk_pr_nct	= "4";  # PrintReads -nct                <== This one is used for PrintReads
	$no_of_threads_gatk_br_nct	= "4";  # BaseRecalibrator -nct          <== This one is used for BaseRecalibrator
	$no_of_threads_gatk_rtc_nt	= "8"; # RealignerTargetCreator         <== This one is used for RealignerTargetCreator
	$workstation 				= "true";
	$e_mail_from 				= 'NGS_analysis_haploid@gen-x1404-ws02.aht.org.uk'; # Who e-mails come from
}
else
...
print "  - When you get your final VCF file you can do two things:\n\n";
print "    1: Use run_variant_effect_predictor (or run_snpEff) to annotate your variants.\n\n";
print "    2: Use vcf2excel_haploid to prepare your VCF file for Excel.\n\n";

print color 'reset';

if ($warning eq "on")
{
	&print_message("fast2vcf_haploid on the Workstation","warning");
...
until ($run_title ne "ls")
{
	...
	{
		print "\n";
		system ("ls *_fastq2vcf_haploid_command_log.out");
		print "\n";
	}
	...
	if ($run_title eq "")
	{
		...
		&print_message ("There needs to be a run title","warning");
		
		print "Run title was not specified so has been set as:  $run_title\n\n";

		print "Press 'return' to continue if this is OK.\n\n";
		$answer=<STDIN>;
	}
}
...
$results_all_folder = "results_fastq2vcf_haploid"."$run_title";
...
$screen_log_file = "$run_title"."_fastq2vcf_haploid_screen_log.out";
...
if (substr($answer,0,1) eq "1" ){$ref = "/home/genetics/strep_equi/strep_equi.fasta"; $ref_seq_name = "s_equi"; $species = "streptococcus_equi";$dummy_dbsnp_file = "/home/genetics/strep_equi/strep_equi_dummy_DBSNP.vcf"; $all_snps_file = "/home/genetics/strep_equi/strep_equi_snps_all.vcf"}
if (substr($answer,0,1) eq "2" ){$ref = "/home/genetics/strep_zoo/strep_zoo.fasta"; $ref_seq_name = "s_zoo"; $species = "streptococcus_zoo";$dummy_dbsnp_file = "/home/genetics/strep_zoo/strep_zoo_dummy_DBSNP.vcf"; $all_snps_file = "/home/genetics/strep_zoo/strep_zoo_snps_all.vcf"}
...
if ($use_defined_region eq "yes")
{
	...
	$GATK_known_sites_string = " -knownSites $all_snps_file";
...
print "   <1> GATK Haplotype Caller using gVCF   (this is what we use for WGS sequences)  [DEFAULT]\n";
...
##########################################################
$command_log = "$run_title"."_fastq2vcf_haploid_command_log.out";
$command_times_log = "$run_title"."_fastq2vcf_haploid_times_log.out";
...
print COMMAND_LOG "Command log of perl script fastq2vcf_haploid version $version\n\n";
...
&print_both("  SNP file for BaseRecalibrator:   \t$all_snps_file\n\n");
...
for ($loop_count=1;$loop_count <=$no_of_files;$loop_count++)
{
	...
	print READMEFILE "###########################################################\n";
	print READMEFILE "Summary of fastq2vcf_haploid results files for this sample     \n";
	print READMEFILE "###########################################################\n\n";
	...
	if ($variant_caller eq "HaplotypeCallerGVCF")
	{
		...
		##########################################################################
		$output_gvcf = "$sample_name".".gVCF";
		$output_gvcf_idx= "$sample_name".".gVCF.idx";
		...
		{
			&run_unix_command("java $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -R $ref -T HaplotypeCaller $ploidy_string --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 $GATK_region_string  -I $final_bam -stand_emit_conf $stand_emit_conf -stand_call_conf $stand_call_conf -o $output_gvcf -S $GATK_validation_stringency -nct $no_of_threads_gatk_ug_nct","Step 14a Run HaplotypeCaller in GVCF mode");
		}
		if ($use_default_stand_values eq "true")
		{
			&run_unix_command("java $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -R $ref -T HaplotypeCaller $ploidy_string  --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 $GATK_region_string  -I $final_bam -o $output_gvcf -S $GATK_validation_stringency -nct $no_of_threads_gatk_ug_nct","Step 14a Run HaplotypeCaller in GVCF mode");	
		}
	...
	print MAIL "Subject: FASTQ2VCF_haploid: Sample $sample_name in run $run_title has reached the final BAM file stage\n\n";
	## Mail Body
	print MAIL "Run title:     \t$run_title\n\n";
	print MAIL "Sample name:   \t$sample_name  (sample number $loop_count)\n\n";
	print MAIL "Script:        \tfastq2vcf_haploid script version $version\n\n";

	print MAIL "FASTQ files:\t$fastq_file_1\t$fastq_file_2\n";
	print MAIL "BAM file:   \t$final_bam\n\n";
	if ($variant_caller eq "HaplotypeCallerGVCF")
	{
		print MAIL "The processing of these FASTQ files into a final BAM file and final gVCF file is complete.\n\n";
	}
	else
	{
...
if ($variant_caller ne "HaplotypeCallerGVCF")

{

	##########################################################
	#  Make a VCF file using all the BAM files in parallel   #
	# (for gVCF option we merge them later using gvcf2vcf)   #
	##########################################################
	...
	if ($variant_caller eq "HaplotypeCaller")
	{
		...
		if ($use_default_stand_values eq "false")
		{
			&run_unix_command("java $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -pairHMM VECTOR_LOGLESS_CACHING -R $ref -T HaplotypeCaller $ploidy_string $GATK_region_string  $variant_caller_input_string -stand_emit_conf $stand_emit_conf -stand_call_conf $stand_call_conf -o $final_VCF_file -S $GATK_validation_stringency -nct $no_of_threads_gatk_ug_nct","18 Run GATK HaplotypeCaller to call variants");	
		}
		if ($use_default_stand_values eq "true")
		{
			&run_unix_command("java $memory_string $temp_dir_string -jar /opt/$gatk_directory/GenomeAnalysisTK.jar -pairHMM VECTOR_LOGLESS_CACHING -R $ref -T HaplotypeCaller $ploidy_string $GATK_region_string  $variant_caller_input_string -o $final_VCF_file -S $GATK_validation_stringency  -nct $no_of_threads_gatk_ug_nct","18 Run GATK HaplotypeCaller to call variants");	
		}
		&record_output_file_size ("$final_VCF_file");
...
{
	########################################################
	# Tell user to use the script gvcf2vcf                 #
	########################################################

	&print_message("Merge your gVCF files using gvcf2vcf ","message");
...
print MAIL "To: $email_address\n";
print MAIL "From: $e_mail_from\n";
print MAIL "Subject: FASTQ2VCF_haploid: Run $run_title has finished\n\n";
## Mail Body
print MAIL "Run title:  $run_title\n\n";
print MAIL "Script:     fastq2vcf_haploid version $version\n\n";
...
print	READMEOVERALL	"==========================================\n";
print	READMEOVERALL	"Summary of fastq2vcf_haploid results files\n";
print	READMEOVERALL	"==========================================\n\n\n";
...
print   READMEOVERALL	"\tIf you made gVCF files you need to do three things:\n\n";
print   READMEOVERALL	"\t\t1: Use gvcf2vcf to merge your gVCF files into a multi-column VCF file.\n\n";
print   READMEOVERALL	"\t\t1: Use run_variant_effect_predictor (or run_snpEff) to annotate your variants.\n\n";
print   READMEOVERALL	"\t\t2: Use vcf2excel_haploid to prepare your VCF file for SNP Handler haploid.\n\n";
...
&print_message("FASTQ2VCF_haploid ANALYSIS COMPLETE! YOU HAVE BEEN SENT AN EMAIL WITH DETAILS","message");
...
print "  For checks on the BAM file(s) produced:\tcheck $validate_final_out and $flagstat_final_out\n\n";


print	"NEXT STEPS\n\n"; 
print   "\tIf you made gVCF files you need to do three things:\n\n";
print   "\t\t1: Use gvcf2vcf to merge your gVCF files into a multi-column VCF file.\n\n";
print   "\t\t1: Use run_variant_effect_predictor (or run_snpEff) to annotate your variants.\n\n";
print   "\t\t2: Use vcf2excel_haploid to prepare your VCF file for SNP Handler haploid.\n\n";
...
sub send_email_err_alert($;$) 
{
	...
	print MAIL "To: $email_address\n";
	print MAIL "From: $e_mail_from\n";
	print MAIL "Subject: FASTQ2VCF_haploid: Error detected in run $run_title\n\n";
	...
	if ($zero_size_reported eq "no")
	{
		if ($filesize < 1)
		{
			...
			open(MAIL, "|/usr/sbin/sendmail -t");
				...
				print MAIL "To: $email_address\n";
				print MAIL "From: $e_mail_from\n";
				print MAIL "Subject: NGS ANALYSIS fastq2vcf_haploid ZERO FILE SIZE: Run $run_title.  Sample $sample_name. File $_outputfile.\n\n";
				## Mail Body
				print MAIL "fastq2vcf_haploid script version $version\n\n";
